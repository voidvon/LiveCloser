...
function getLiveKitStartGuard(): Pick<AppConfig, 'sessionStartEnabled' | 'sessionStartDisabledReason'> {
  ...
  if (isPlaceholder(LIVEKIT_URL)) {
    problems.push('LIVEKIT_URL 还是占位值');
  }
  if (isPlaceholder(LIVEKIT_API_KEY)) {
    problems.push('LIVEKIT_API_KEY 还是占位值');
  }
  if (isPlaceholder(LIVEKIT_API_SECRET)) {
    problems.push('LIVEKIT_API_SECRET 还是占位值');
  }

  if (problems.length === 0) {
    return {
      sessionStartEnabled: true,
      sessionStartDisabledReason: undefined,
    };
  }

  return {
    sessionStartEnabled: false,
    sessionStartDisabledReason:
      `当前前端还没有连接到真实 LiveKit 服务：${problems.join('，')}。` +
      ' 先把 frontend/.env.local 里的 LiveKit 服务地址、API Key、API Secret 换成真实值，再启动会话。',
  };
}
